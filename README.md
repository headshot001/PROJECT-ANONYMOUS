...
**6) `system/etc/hosts` SHOWS READ-ONLY FILE SYSTEM ALTHOUGH THE SCRIPT IS RUNNING UNDER SUDO MODE -** _TO FIX THIS OPEN TERMUX AND TYPE `tsu && mount -o rw,remount /system` AND HIT ENTER THEN OPEN MT MANAGER AND GO TO `system/etc/hosts` LONG CLICK ON IT CLICK ON 3 DOTS ON THE UPPER RIGHT CORNER AND SELECT PERMISSION.THEN SET PERMISSION TO `644`.DO THIS EVERTIME BEFORE EXECUTING THE SCRIPT_
